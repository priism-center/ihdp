...
>$covs_nr
treat ~ bwg + hispanic + black + b.marr + lths + hs + ltcoll + 
    work.dur + prenatal + sex + first + bw + income + preterm + 
    momage + dayskidh

>$covs_nr_st
treat ~ bwg + hispanic + black + b.marr + lths + hs + ltcoll + 
    work.dur + prenatal + sex + first + bw + income + preterm + 
    momage + dayskidh + st5 + st9 + st12 + st25 + st36 + st42 + 
    st53

>$covsi21
treat ~ bw + preterm + dayskidh + sex + hispanic + b.marr + lths + 
    hs + ltcoll + work.dur + prenatal + momage + income + bwT + 
    pretermT + income + black:dayskidT + b.marr:bw + b.marr:preterm + 
    b.marr:dayskidT + bw:income

>$covsi21_st
treat ~ bw + preterm + dayskidh + sex + hispanic + b.marr + lths + 
    hs + ltcoll + work.dur + prenatal + momage + income + bwT + 
    pretermT + st5 + st9 + st12 + st25 + st36 + st42 + st53 + 
    black:dayskidT + bw:b.marr + preterm:b.marr + b.marr:dayskidT + 
    bw:income
...
overlap_mwor <- map2(matched_mwor, names(matched_mwor), function(df=.x, name=.y, bins=100){
    p <- ggplot(df) +
        geom_histogram(aes(ps, fill=factor(treat)), col='black', position='stack', bins=bins) +
        treat_fill +
        labs(x='pscore', title=paste0('MwoR: ', name))
})
...
overlap_mwr <- map2(matched_mwr, names(matched_mwr), function(df=.x, name=.y, bins=100){
    p <- ggplot(df) +
        geom_histogram(aes(ps, fill=factor(treat)), col='black', position='stack', bins=bins) +
        treat_fill +
        labs(x='pscore', title=paste0('MwR: ', name))
})
