...
```{r}
# every subset of covs.nr for te spec
covs.nr
n <- length(covs.nr)
...
summary(sapply(ests, function(est) est[1]))
summary(sapply(ests, function(est) est[2]))

hist(sapply(ests, function(est) est[1]))
hist(sapply(ests, function(est) est[2]))
# no luck in the MWR samples
```

```{r}
# stepwise for ps
ps_spec_null <- formula(treat ~ 1)

set.seed(8)
ps_fit_null <- glm(ps_spec_null, family=binomial(link='logit'), data=cc2[, c('treat', 'bwg', 'educ', 'ethnic', 'b.marr', 'work.dur', 'prenatal', 'preterm', 'momage', 'sex', 'first', 'bw', 'dayskidT', 'pretermT', 'momageT')])
ps_fit_step <- step(ps_fit_null, scope = list(upper=as.formula(treat ~ .^2), lower=as.formula(treat ~ .)), direction = "both")
```
