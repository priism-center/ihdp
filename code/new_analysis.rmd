...
colnames(tes.ns) <- c('lths', 'hs', 'ltcoll', 'college')
rownames(tes.ns) <- c('te', 'n.trt', 'n.ctrl', 'se')
round(tes.ns,2)
```
```{r}
# Stratified analyses
...
tes.ns[1,] <- c(
    mean(cc2$ppvtr.36[treated & lths]) - mean(cc2$ppvtr.36[!treated & lths]),
    sum(treated & lths),
    sum(lths),
    sqrt(var(cc2$ppvtr.36[treated & lths]) / sum(treated & lths) + var(cc2$ppvtr[treated & lths] / sum(!treated & lths)))
```
